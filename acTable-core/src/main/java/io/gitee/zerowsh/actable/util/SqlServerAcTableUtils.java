...
public class SqlServerAcTableUtils {
    ...
    public static List<String> getCreateTableSql(TableInfo tableInfo) {
        ...
        for (TableInfo.PropertyInfo propertyInfo : propertyInfoList) {
            ...
            propertySb.append(StringPool.LEFT_SQ_BRACKET).append(columnName).append(StringPool.RIGHT_SQ_BRACKET);
        ...
        //建表
        resultList.add(StrUtil.format(CREATE_TABLE, tableName, propertySb.deleteCharAt(propertySb.length() - 1)));
        ...
        for (TableColumnInfo tableColumnInfo : tableColumnInfoList) {
            ...
            while (it.hasNext()) {
                ...
                if (Objects.equals(tableColumnInfo.getColumnName(), propertyInfo.getColumnName())) {
                    ...
                    boolean judgeDef = defExistUpdate || (!(Objects.equals(tableColumnInfo.getTypeStr(), propertyInfo.getType().getType())) && Objects.nonNull(tableColumnInfo.getDefaultValue()));
                    if (judgeDef) {
    ...
    private static void splicingColumnInfo(StringBuilder propertySb, TableInfo.PropertyInfo propertyInfo,
        ...
        if (StrUtil.isNotBlank(propertyInfo.getDefaultValue())) {
            if (isUpdate) {
                ...
                propertySb.append(StringPool.SPACE).append(DEFAULT).append(StringPool.SPACE).append(propertyInfo.getDefaultValue());
            }
        }

        //是否为空
        if (propertyInfo.isNull() && !propertyInfo.isKey() && !propertyInfo.isAutoIncrement()) {
            propertySb.append(StringPool.SPACE).append(NULL);
        } else {
            propertySb.append(StringPool.SPACE).append(NOT_NULL);
        }
    ...
    private static void splicingColumnType(StringBuilder propertySb, TableInfo.PropertyInfo propertyInfo, String tableName) {
        ...
        switch (type) {
            ...
            case NCHAR:
            case CHAR:
                propertySb.append(StringPool.SPACE).append(type.getType()).append(StringPool.LEFT_BRACKET);
                if (Objects.equals(type, ColumnTypeEnums.DATETIME2)) {
                    //对类型特殊处理
                    if (length > 7 || length < 0) {
                        log.warn(COLUMN_LENGTH_VALID_STR, tableName, columnName, type, length, 7);
                        propertySb.append(7);
                    } else {
            ...
            case NUMERIC:
                ...
                break;
            default:
                propertySb.append(StringPool.SPACE).append(type.getType());
        }
    }
