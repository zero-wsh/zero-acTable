...
public interface SqlConstants {
    ...
    static String getExecuteSql(SqlTypeEnums sqlTypeEnums) {
        ...
        switch (databaseType) {
            case MYSQL:
                sql = MYSQL_EXECUTE_SQL.get(sqlTypeEnums);
                break;
            case SQL_SERVER:
                sql = SQL_SERVER_EXECUTE_SQL.get(sqlTypeEnums);
                break;
            default:
                throw new RuntimeException(StrUtil.format("数据库类型不支持 databaseType={}", databaseType));
        }
        if (StrUtil.isBlank(sql)) {
            throw new RuntimeException(StrUtil.format("获取执行sql异常 databaseType={} sqlTypeEnums={}", databaseType, sqlTypeEnums));
        }
        return sql;
    }
}
