    ...
    public List<String> getCreateTableSql(TableInfo tableInfo) {
        ...
        if (CollectionUtil.isNotEmpty(keyList)) {
            ...
            for (String key : tableInfo.getKeyList()) {
                pkSb.append(StrUtil.format(MYSQL_KEYWORD_HANDLE, key)).append(StrUtil.COMMA);
            }
            propertySb.append(StrUtil.format(PRIMARY_KEY, pkSb.deleteCharAt(pkSb.length() - 1))).append(StrUtil.COMMA);
        }
...
//                uniqueSb.append(StrUtil.format(MYSQL_KEYWORD_HANDLE, column)).append(StrUtil.COMMA);
//            }
//            propertySb.append(StrUtil.format(UNIQUE_KEY, uniqueInfo.getValue(), uniqueSb.deleteCharAt(uniqueSb.length() - 1))).append(StrUtil.COMMA);
        }
        for (TableInfo.IndexInfo indexInfo : tableInfo.getIndexInfoList()) {
//            String[] columns = indexInfo.getColumns();
//            StringBuilder indexSb = new StringBuilder();
//            for (String column : columns) {
//                indexSb.append(StrUtil.format(MYSQL_KEYWORD_HANDLE, column)).append(StrUtil.COMMA);
//            }
//            propertySb.append(StrUtil.format(INDEX_KEY, indexInfo.getValue(), indexSb.deleteCharAt(indexSb.length() - 1))).append(StrUtil.COMMA);
        }
    ...
    private static void createPk(Set<String> delConstraintSet, TableInfo tableInfo, boolean tableExistPk) {
        ...
        //添加主键
        if (CollectionUtil.isNotEmpty(keyList)) {
            if (!tableExistPk || delConstraintSet.contains(MYSQL_DEL_PK)) {
                StringBuilder keySb = new StringBuilder();
                for (String key : keyList) {
                    keySb.append(StrUtil.format(MYSQL_KEYWORD_HANDLE, key)).append(StrUtil.COMMA);
                }
...
//                delConstraintSet.add(StrUtil.format(MYSQL_ADD_INDEX, value, uniqueSb.deleteCharAt(uniqueSb.length() - 1)));
    ...
    public String getAllTableSql() {
        return "select table_name as name from information_schema.tables where table_schema = (select database())";
    }

    @Override
    public String existTableSql(String tableName) {
        return "select count(1) from information_schema.tables where table_name ='{}' and table_schema = (select database())";
    }

    @Override
    public String getTableStructureSql(String tableName) {
        return StrUtil.format("SELECT t.table_name tableName,t.table_comment tableComment," +
                ...
                " and c.table_schema = (select database()) AND t.table_schema = (SELECT DATABASE ())", tableName);
    }

    @Override
    public String getConstraintInfoSql(String tableName) {
        return StrUtil.format("select index_name constraintName ,GROUP_CONCAT(column_name order by column_name) constraintColumnName," +
    ...
    public static boolean handleUkList(List<ConstraintInfo> constraintInfoList, TableInfo.UniqueInfo uniqueInfo) {
        ...
        while (it.hasNext()) {
            ...
            if (Objects.equals(constraintInfo.getConstraintFlag(), UK)) {
//                String[] columns = uniqueInfo.getColumns();
//                String[] constraintColumnName = constraintInfo.getConstraintColumnName().split(StrUtil.COMMA);
